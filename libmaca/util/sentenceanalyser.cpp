...
namespace Maca {
	...
	SentenceAnalyser::SentenceAnalyser(boost::shared_ptr<Toki::Tokenizer> tok,
	...
	SentenceAnalyser::SentenceAnalyser(const Config::Node &cfg)
		: UnicodeSink()
		, tok_(Toki::LayerTokenizer::create_from_named_config(cfg.get("general.toki-config", "")))
		, sp_(*tok_)
		, ma_(new DispatchAnalyser(cfg))
	{
	}

	SentenceAnalyser::SentenceAnalyser(const Config::Node &cfg, const Toki::Config::Node &toki_config_override)
		: UnicodeSink()
	...
	boost::shared_ptr<SentenceAnalyser> SentenceAnalyser::create_from_named_config(std::string config_name)
	{
		const Config::Node& cfg = get_named_config(config_name);
		return boost::make_shared<SentenceAnalyser>(cfg);
	}

	boost::shared_ptr<SentenceAnalyser> SentenceAnalyser::create_from_named_config(std::string config_name, std::string toki_config_override)
	{
		const Config::Node& cfg = get_named_config(config_name);
		const Toki::Config::Node& toki_cfg = Toki::get_named_config(toki_config_override);
	...
	{
		if (sp_.has_more()) {
			boost::scoped_ptr<Toki::Sentence> toki_sentence(sp_.get_next_sentence());
